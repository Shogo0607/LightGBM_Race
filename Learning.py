...
if vote == "1,2":
    ...
    y_train = train[["単勝配当"]]
    X_valid = test[["頭数","生後日数","騎手指数","見習い騎手","種牡馬","父系統","母父系統","父年齢","母年齢","血統距離適性","厩舎ランク","調教師","生産者","馬主","蹄","斤量","斤量構成比","馬体重","馬体重増減","重適性","スタート指数","出遅れ率","出遅れ","直線不利","予想IDM","1走前IDM","ベストIDM","中央IDM","1走前着順評価","頭数変化","場所変化","開催変化","負担重量変化","1走前着差","芝ダ変化","最長距離","去勢","距離変化","コース脚質","内_脚質","中_脚質","外_脚質","内_道中","中_道中","外_道中","内_直線","中_直線","外_直線","1走前3コーナー","1走前コーナー順位","テン指数","上がり指数","コース枠番","奇数偶数","調教素点","厩舎素点","CID素点","CID","調教評価","厩舎評価","追切指数","終いF指数","仕上指数","調教パターン","輸送","外厩","放牧先ランク","休養理由","競走馬レベル","間隔","獲得賞金","1走前支持率","ベスト支持率","中央支持率","JRA通算連対率","JRA通算複勝率","キャリア","騎手期待単勝率","騎手期待連対率","騎手期待複勝率","激走指数","LS指数","万券指数","基準支持率"]]
    y_valid = test[["単勝配当"]]
...
with st.spinner("データ分析中"):
    ...
    filename = "投票_" + vote + "_的中率_" + str(int(acc_rate)) + "_回収率_"+ str(int(pay_back)) + '.pkl'

    data = pickle_model(model)
    st.download_button("Download .pkl file", data=data, file_name=filename)